...
func NewHgssSavefile(bytes []byte) *HgssSavefile {
	sbStart, sbEnd := uint(0x0), uint(0xF627)
...
func (hgss *HgssSavefile) parsePokemon(index int) models.Pokemon {
	offset := 0x98 + index*236
	savefile := hgss.latestSave.Data()
	raw := crypt.DecryptPokemon(savefile[offset : offset+236])
	hgss.rawParty = append(hgss.rawParty, raw...)
...
func (hgss *HgssSavefile) PartyPokemon() []*models.Pokemon {
	partySize := int(utils.U32(hgss.latestSave.Data(), 0x98-0x4))
	for i := range partySize {
		pkmn := hgss.parsePokemon(i)
		hgss.partyPokemon = append(hgss.partyPokemon, &pkmn)
	}
	return hgss.partyPokemon
}

func (hgss *HgssSavefile) validatePokemon(p *models.Pokemon) error {
	...
	_, ok := itemMap[p.HeldItem]
	if !ok {
		return newError("invalid item '%s'", p.HeldItem)
	} else if p.HeldItem == "Explorer Kit" {
		return newError("item '%s' not supported in HGSS", p.HeldItem)
	}
	...
	// EXP validation
	growthType := hgss.pokemonMetadata[p.PokedexId].GrowthType
	// contains total EXP required to reach each level (has an entry for lvl 0 for some reason)
	expTable := hgss.expTable[growthType]
...
func (hgss *HgssSavefile) validate() error {
	if len(hgss.partyPokemon) > 6 {
		return fmt.Errorf("VALIDATION ERR: party cannot hold more than 6 pokemon")
	}

	for _, p := range hgss.partyPokemon {
		if err := hgss.validatePokemon(p); err != nil {
			return err
		}
	}
...
func (hgss *HgssSavefile) updatePokemon(index int, p *models.Pokemon) {
	start := index * 236
	buf := hgss.rawParty[start : start+236]
	pid := utils.U32(buf, 0)
...
func (hgss *HgssSavefile) Flush() error {
	...
	for i, p := range hgss.partyPokemon {
		hgss.updatePokemon(i, p)
		offset := i * 236
		ciphertext := crypt.EncryptPokemon(hgss.rawParty[offset : offset+236])
		encryptedBuffer = append(encryptedBuffer, ciphertext...)
	}

	copy(hgss.latestSave.Data()[0x98:], encryptedBuffer)

	newChecksum := crypt.CRC16_CCITT(hgss.latestSave.Data())
	hgss.latestSave.Footer.Checksum = newChecksum

	copy(hgss.rawBytes[hgss.latestSave.Offset():], hgss.latestSave.Bytes())

	fullpath := path_resolver.GetRoot() + "/new-hgss.sav"
	os.WriteFile(fullpath, hgss.rawBytes, os.ModePerm)
	return nil
}
