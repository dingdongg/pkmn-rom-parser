...
func NewDpSavefile(bytes []byte) *DpSavefile {
	sbStart, sbEnd := uint(0x0), uint(0xC0FF)
...
func (dp *DpSavefile) parsePokemon(index int) models.Pokemon {
	offset := 0x98 + index*236
	savefile := dp.latestSave.Data()
	raw := crypt.DecryptPokemon(savefile[offset : offset+236])
	dp.rawParty = append(dp.rawParty, raw...)
...
func (dp *DpSavefile) PartyPokemon() []*models.Pokemon {
	partySize := int(utils.U32(dp.latestSave.Data(), 0x98-0x4))
	for i := range partySize {
		pkmn := dp.parsePokemon(i)
		dp.partyPokemon = append(dp.partyPokemon, &pkmn)
	}
	return dp.partyPokemon
}

func (dp *DpSavefile) validatePokemon(p *models.Pokemon) error {
	...
	dpBlacklist := map[string]bool{}
	dpBlacklist["Griseous Orb"] = true
	dpBlacklist["Vs. Recorder"] = true
	dpBlacklist["Gracidea"] = true
	dpBlacklist["Secret Key"] = true
	if !ok {
		return newError("invalid item '%s'", p.HeldItem)
	} else if item.Exclusivity == "HGSS" {
		return newError("item '%s' not supported in DP", p.HeldItem)
	} else if _, ok = dpBlacklist[p.HeldItem]; ok {
		return newError("item '%s' not supported in DP (plat and onwards)", p.HeldItem)
	}
	...
	if !ok {
		...
		return newError("ability '%s' not supported in DP", p.Ability)
	}
...
func (dp *DpSavefile) validate() error {
	if len(dp.partyPokemon) > 6 {
		return fmt.Errorf("VALIDATION ERR: party cannot hold more than 6 pokemon")
	}

	for _, p := range dp.partyPokemon {
		if err := dp.validatePokemon(p); err != nil {
			return err
		}
	}
...
func (dp *DpSavefile) updatePokemon(index int, p *models.Pokemon) {
	start := index * 236
	buf := dp.rawParty[start : start+236]
	pid := utils.U32(buf, 0)
...
func (dp *DpSavefile) Flush() error {
	...
	for i, p := range dp.partyPokemon {
		dp.updatePokemon(i, p)
		offset := i * 236
		ciphertext := crypt.EncryptPokemon(dp.rawParty[offset : offset+236])
		encryptedBuffer = append(encryptedBuffer, ciphertext...)
	}

	copy(dp.latestSave.Data()[0x98:], encryptedBuffer)

	newChecksum := crypt.CRC16_CCITT(dp.latestSave.Data())
	dp.latestSave.Footer.Checksum = newChecksum

	copy(dp.rawBytes[dp.latestSave.Offset():], dp.latestSave.Bytes())

	fullpath := path_resolver.GetRoot() + "/new-dp.sav"
	os.WriteFile(fullpath, dp.rawBytes, os.ModePerm)
	return nil
}
